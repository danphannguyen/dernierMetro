function nextArrival(now = new Date(), n = 1) {
  ...
  if (!headwayMin || headwayMin <= 0) return { arrivals: [], tz };
	...
	// Créer les objets Date pour lastWindow et end
	...
	// si end est avant ou égal à lastWindow, ça passe au lendemain
	if (end <= lastWindow) {
	...
	for (let i = 1; i <= n; i++) {
		...
		const isLast =
			new Date(now.getTime() + headwayMin * (i + 1) * 60 * 1000) >= end;

		arrivals.push({ time: toHM(nextTime), isLast });
	}
